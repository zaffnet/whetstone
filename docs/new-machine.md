...
2. Run the bootstrap:
   ...
   Brewfile, macOS defaults, pinned agent skills.
...
4. Work machine only: open `~/.config/chezmoi/chezmoi.toml` and fill the `[data.work]`
   table (`openai_base_url`, `aws_profile`, `pypi_index_url`, `pypi_publish_url`,
   `branch_prefix`), then `chezmoi apply`. Employer-specific shell lines go in
   `~/.zshrc.local`; git signing and CA bundle settings go in `~/.gitconfig.local`.
...
3. Back up the files chezmoi will replace: `chezmoi managed | xargs -I{} cp -a ~/{} ~/backup/`
   (or rely on `chezmoi apply --dry-run --verbose` plus git history).
4. `chezmoi apply`.
5. Codex rewrites its runtime sections (`[projects.*]`, `[marketplaces.*]`, bundled MCP
   servers) on next launch and asks again to trust each project directory. Claude Code
   keeps `~/.claude.json` untouched; run `~/.agents/bin/sync-mcp` once so its MCP list
   matches `~/.agents/mcp.json`.
6. Run `just test` with `WHETSTONE_HOME=$HOME` to confirm the symlinks resolve.

## CI mode

With `CI=1` set, `.chezmoiignore` skips the Oh My Zsh, Homebrew, and agent-skills scripts so
the tree can be applied into a throwaway `HOME` in seconds. `just test-home` does exactly
that locally and runs the bats suite against the result.
